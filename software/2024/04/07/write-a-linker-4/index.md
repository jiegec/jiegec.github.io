...
本文同步发布到本人的[知乎](https://zhuanlan.zhihu.com/p/1934369623450580605)。
...
1. 规划将要生成的 ELF 可执行文件的内容布局：开始是固定的文件头，之后是各个 section，计算出它们从哪里开始到哪里结束
1. 如果要生成动态库，还需要针对动态链接器，构造一些额外的 section（`.dynamic`，`.dynsym` 和 `.dynstr`）
1. 第二步完成以后，就可以知道在运行时，各个 section 将会被加载到哪个地址上，进而更新符号表，得到每个符号在运行时会处在哪个地址上；此时我们就可以计算重定位，把地址按照预设的规则填入到对应的地方
1. 如果要生成动态库，因为运行时动态库的加载地址不确定，所以在 ELF 中把加载地址设为 0，并且要求代码是地址无关代码（PIC，Position Independent Code）
1. 最后按照预设的文件布局，把文件内容写入到 ELF 文件中
...
1. `.dynsym`：Dynamic Symbol，和动态链接有关的动态符号表
1. `.dynstr`：Dynamic String，和动态链接有关的字符串的表
1. `.dynamic`：Dynamic，向动态链接器提供了一些信息
...
1. `.plt`：Procedure Linkage Table，涉及到动态链接库的函数，下面会详细介绍
1. `.rela.plt`：针对 Procedure Linkage Table 的重定位（Relocation）信息，下面会详细介绍
1. `.got.plt`：针对 Procedure Linkage Table 的 Global Offset Table，下面会详细介绍
...
1. `DEBUG`：用于调试，这里不讨论
1. `PLTGOT`：指向了 `.got.plt` 段的地址
1. `PLTRELSZ`：记录了 `.rela.plt` 每一项的大小
1. `PLTREL`：`RELA` 表示 `.rela.plt` 每一项里包含 addend 项（RELA = REL with Addend）
1. `JMPREL`：指向了 `.rela.plt` 段的地址
...
答案是还不满足于当前的性能：完整的代码里可能会调用动态库里的一千个函数，但是实际运行的时候，并非所有代码都会被执行，于是可能只有一小部分动态库的函数会被调用。这时候，就希望实现一点：第一次调用动态库里的函数的时候，再进行重定位，之后就一直用这个重定位好的结果。那么如果一个函数从来没有被调用过，那就省下了重定位的时间。这就是惰性（Lazy）或者说延迟（Deferred）的思想。

为了实现这个功能，它是这么实现的：

1. 第一次调用的时候，会跳转到动态链接器里，让动态链接器完成重定位，然后跳转到实际的动态库里的函数
1. 第二次和之后调用的时候，直接跳转到实际的动态库里的函数
...
1. 执行 `401016: 68 00 00 00 00 push $0x0`，向栈上压入了 `0`，具体什么含义，下面会进行分析
1. 执行 `40101b: e9 e0 ff ff ff jmp 401000 <print@plt-0x10>`，跳转到 `0x401000`
1. 执行 `401000: ff 35 ea 1f 00 00 push 0x1fea(%rip) # 402ff0 <_GLOBAL_OFFSET_TABLE_+0x8>`，具体什么含义，下面会进行分析
1. 执行 `401006: ff 25 ec 1f 00 00 jmp *0x1fec(%rip) # 402ff8 <_GLOBAL_OFFSET_TABLE_+0x10>`，这一条指令跳转到 `0x402ff8` 地址保存的内容，这个内容就是动态链接器提供的函数（如果是 glibc 提供的动态链接器，这个函数是 [\_dl_runtime_resolve](https://github.com/bminor/glibc/blob/master/sysdeps/x86_64/dl-trampoline.h)），它会负责进行重定位，重定位以后，就会把结果写到 `0x403000` 地址里
...
1. 压入了 `0x402ff0` 地址的内容，这个内容由动态链接器初始化，可以想到大概是用来表示当前可执行文件的某个指针
...
1. 0x402ff0 开始的 8 个字节：保留给动态链接器
1. 0x402ff8 开始的 8 个字节：保留给动态链接器
1. 0x403000 开始的 8 个字节：`print` 的地址，初始化为 `0x401016`
1. 0x403008 开始的 8 个字节：`exit` 的地址，初始化为 `0x401026`
...
1. 为了减少重定位次数，在 PLT 里给每个要调用的动态库里的函数提供了一个跳板函数
1. 为了实现惰性重定位，第一次调用跳板函数，会调用动态链接器的函数来实现重定位，通过压栈来区分要重定位的函数
1. 惰性重定位完成以后，跳板函数直接 jmp 到目标函数
...
1. 生成 .plt 和 .got.plt 段的开头
   1. .plt 开头要生成 push，jmp 和 nop 的指令序列
   1. .got.plt 段开头保存 .dynamic 段的地址，然后预留 16 字节给动态链接器
1. 要调用动态链接库里的函数的时候，在 PLT 和 GOT 里生成对应的项：
   1. .plt 里要生成 jmp，push 和 jmp 三条指令的序列
   1. .got.plt 要添加用于保存实际地址的一项
   1. .rela.plt 也相应地更新，指向 .got.plt 里对应项的地址
1. 根据命令行参数，设置 DT_INTERP；对于依赖的动态库，添加 DT_NEEDED
1. 把 .plt、.got.plt 相关的信息记录到 .dynamic 段中
