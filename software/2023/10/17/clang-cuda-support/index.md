...
1. `ptxas --gpu-name sm_70 --output-file axpy-sm_70.o axpy-sm_70.s`：使用 CUDA 提供的 `ptxas` 程序，把 PTX 汇编翻译成 SM 70 的 SASS 指令，打包到一个 ELF object 中
1. `fatbinary --create axpy.fatbin --image=profile=sm_70,file=axpy-sm_70.o --image=profile=compute_70,file=axpy-sm_70.s`: 使用 CUDA 提供的 `fatbinary` 程序，把 ptxas 生成的 ELF object 和 clang 生成的 PTX 汇编内容打包成一个 fatbin 文件
1. `clang -triple x86_64-pc-linux-gnu -emit-obj axpy.cu -fcuda-include-gpubinary axpy.fatbin -o axpy.o`：编译 Host 代码，把 fatbin 的内容嵌入到一个 `.nv_fatbin` section 中，生成一个 ELF object 中
1. `ld -o axpy axpy.o -lcudart`：最后一步就是常规的链接，得到最终的可执行文件

实际上还有一步预处理，这里省略了。对比通常 C++ 程序的编译流程，这里不同的点在于：

1. 用 Clang 以 NVPTX target 编译了一遍，得到 PTX 汇编
1. 用 CUDA 的 ptxas，把 PTX 汇编翻译成对应 SM Architecture 的 SASS 指令，放到一个 ELF object 里
1. 用 CUDA 的 fatbinary，把第一步和第二步的结果放到一个 fatbin 文件里
1. 把 fatbin 文件嵌入到 ELF 中
...
1. 接下来是若干个 entry:
   1. 每个 entry 有一个头部
   1. 每个 entry 有一个 payload，里面是 ELF 或者 PTX，可选压缩
