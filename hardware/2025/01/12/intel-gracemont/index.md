...
猜测它是一个 26 路全相联的设计，每个表项有 16-bit 的 tag（取自 Load PC 低 16 位）和 2-bit 饱和计数器。未命中时预测无依赖，如果实际有依赖则插入表项，计数器置零。命中时根据计数器值预测：无依赖加一，有依赖置零，到 3 时预测无依赖。另一种可能是计数器到 3 时表项被删除，效果相同。究竟是哪种还有待后续研究。这种设计和 ARM Neoverse N2 比较类似。
