...
也就是说，它的 4K 60Hz 从输入侧已经是 YCbCr 4:2:0 了，也就是每 2x2 的四个像素里，有四个 Y，一个 Cb 和一个 Cr。这样平均下来，8-bit 深度下每个像素的空间是 ((4\*8+8+8)/4 = 12) bit。如果是 4:2:2 的话，每 2x2 的四个像素里，有四个 Y，两个 Cb 和两个 Cr，平均下来，8-bit 深度下每个像素的空间是 ((4\*8+2\*8+2\*8)/4 = 16) bit。如果直接保存 RGB，8-bit 深度下就是 (3\*8=24) bit。
